    ...
    <Box sx={{bgcolor: 'primary.main'}}>
      ...
      {[...Array(6)].map((_, index) => (
        <CustomTabPanel key={index} value={value} index={index}>
          {index === 0 ? (
           data.map((item,index)=>(
<TechHubCom img={item.img} name={item.name} address={item.address} date={item.date} blogname={item.blogname} blogtext={item.blogtext} likecount={item.likecount} commentcount={item.commentcount} sharecount={item.sharecount} islike={item.islike}/>
           ))
          ) : (
